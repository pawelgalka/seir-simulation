...
import static pl.agh.kis.seirsimulation.model.configuration.Configuration.BIRTH_RATE;
import static pl.agh.kis.seirsimulation.model.configuration.Configuration.CONTACT_RATE;
import static pl.agh.kis.seirsimulation.model.configuration.Configuration.DEATH_RATE;
...
public class SeirStrategy implements DiseaseStrategy {
    ...
    public int calculateSusceptibleChange(List<Integer> stateCount, double stateCountSum,
        ...
        return (int) floorOrCeil((BIRTH_RATE * stateCountSum) - (DEATH_RATE * stateCount.get(0)) - (
                (CONTACT_RATE() * stateCount.get(2) * stateCount.get(0)) / stateCountSum));
    }

    @Override
    public int calculateExposedChange(List<Integer> stateCount, double stateCountSum, boolean lessThanHundredInfected) {
        return (int) floorOrCeil((CONTACT_RATE() * stateCount.get(2) * stateCount.get(0)) / stateCountSum
                - (DEATH_RATE + 1 / DISEASE_CONFIG
    ...
    public int calculateRecoveredChange(List<Integer> stateCount, boolean lessThanHundredInfected) {
        return (int) floorOrCeil(
                (1 / DISEASE_CONFIG.getInfection()) * stateCount.get(2) - (DEATH_RATE * stateCount.get(3)));
    }
    ...
    public int[] getDailyChanges(Cell cell) {
        ...
        double stateCountSum = (double) stateCount.stream().mapToInt(Integer::intValue).sum();
