...
public class CSVWriter {
    ...
    private Consumer<OutputDataDto> csvDataConsumer(CSVPrinter printer) {
        return dataEntry -> {
            try {
                printer.printRecord(dataEntry.getDay(), dataEntry.getSusceptible(), dataEntry.getExposed(),
                        dataEntry.getInfectious(),
    ...
    private void createPdfRaport(String csvFileName) throws IOException, InterruptedException {
        String currentRelativePath = Paths.get("").toAbsolutePath() + "\\raport.py";
        csvFileName = Paths.get("").toAbsolutePath() + "\\" + csvFileName;
